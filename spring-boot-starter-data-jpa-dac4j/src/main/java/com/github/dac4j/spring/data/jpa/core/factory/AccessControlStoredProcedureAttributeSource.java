...
/**
 * A factory class for {@link AccessControlStoredProcedureAttributes}.
 *
 * @author Fahd Arhalai
 */
enum AccessControlStoredProcedureAttributeSource {

	INSTANCE;

	/**
	 * Creates a new {@link AccessControlStoredProcedureAttributes} from the given {@link Method} and {@link JpaEntityMetadata}.
	 *
	 * @param method must not be {@literal null}
	 * @param entityMetadata must not be {@literal null}
	 * @return
	 */
	public AccessControlStoredProcedureAttributes createFrom(Method method, JpaEntityMetadata<?> entityMetadata) {
		...
		return new AccessControlStoredProcedureAttributes(procedureName, createOutputProcedureParameterFrom(method, procedure));
	}
	 ...
	 * @return
	 */
	private AccessControlStoredProcedureAttributes newProcedureAttributesFrom(Method method, NamedStoredProcedureQuery namedStoredProc,
																			  Procedure procedure) {

		List<AccessControlProcedureParameter> outputParameters;
		...
		if (!procedure.outputParameterName().isEmpty()) {
			...
			outputParameters = extractOutputParametersFrom(namedStoredProc).stream() //
					.map(namedParameter -> new AccessControlProcedureParameter(namedParameter.name(), namedParameter.mode(),
							namedParameter.type())) //
					.collect(Collectors.toList());
		}

		return new AccessControlStoredProcedureAttributes(namedStoredProc.name(), outputParameters, true);
	}

	/**
	 * Create a {@link AccessControlProcedureParameter} from relevant {@link Method} and {@link Procedure}.
	 *
	 * @param method
	 * @param procedure
	 * @return
	 */
	private AccessControlProcedureParameter createOutputProcedureParameterFrom(Method method, Procedure procedure) {

		return new AccessControlProcedureParameter(procedure.outputParameterName(),
